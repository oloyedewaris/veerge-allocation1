...
import { ChevronDown, Cuboid, Menu as MenuIcon, Minus, Plus, Rotate3D } from "lucide-react";
import { Suspense, useEffect, useMemo, useRef, useState } from "react";
import type { OrbitControls as OrbitControlsImpl } from "three-stdlib";
import * as THREE from "three";

const ROOT = "/reference-assets/";
type Unit = { block:string; id:string; model:string; landArea:string; buildArea:string; bedrooms:string; bathrooms:string; master:string; living:string; kitchen:string; guest:string; direction:string; availability:string };

const MODEL_DATA = {
  A: { model:"date/objects3d/43/345-Type A.glb", textures:["date/textures/36/251-Type_A_Building_web.jpg","date/textures/37/254-Type_A_Details_web.jpg","date/textures/38/263-Type_A_Inner_Walls_web.jpg","date/textures/39/266-Type_A_Tree_web.jpg"], order:["building","details","metal","glass","metalDark","clear","tree","inside"] },
  B: { model:"date/objects3d/44/312-Type_B.glb", textures:["date/textures/41/273-Type_B_Building_web.jpg","date/textures/42/274-Type_B_Details_web.jpg","date/textures/43/277-Type_B_Inner_Walls_web.jpg","date/textures/44/280-Type_B_Tree_web.jpg"], order:["details","metal","glass","metalDark","tree","clear","building","inside"] },
  C: { model:"date/objects3d/45/314-Type_C.glb", textures:["date/textures/45/283-Type_C_Building_web.jpg","date/textures/46/286-Type_C_Details_web.jpg","date/textures/47/290-Type_C_Inner_Walls_web.jpg","date/textures/48/293-Type_C_Tree_web.jpg"], order:["building","details","glass","metal","clear","metalDark","inside","tree"] }
} as const;
...
function VillaModel({ type }: { type:ModelType }) {
  ...
  const maps = useLoader(THREE.TextureLoader, data.textures.map((path) => `${ROOT}${encodeURI(path)}`));
  const scene = useMemo(() => {
    maps.forEach((map) => { map.colorSpace=THREE.SRGBColorSpace; map.flipY=false; map.needsUpdate=true; });
    const textured = (map:THREE.Texture) => new THREE.MeshPhongMaterial({ map, color:"#fff", specular:"#111", shininess:30 });
    const materials:Record<string,THREE.Material> = {
      building:textured(maps[0]), details:textured(maps[1]), inside:textured(maps[2]), tree:textured(maps[3]),
      metal:new THREE.MeshStandardMaterial({color:"#050505",roughness:0,metalness:1}),
      metalDark:new THREE.MeshStandardMaterial({color:"#3f3f3f",emissive:"#3f3f3f",roughness:0,metalness:1}),
      glass:new THREE.MeshPhongMaterial({color:"#fff",specular:"#fff",shininess:0,opacity:.2,transparent:true}),
      clear:new THREE.MeshPhongMaterial({color:"#cfcfcf",specular:"#30a7ff",shininess:27.6,opacity:.25,transparent:true})
    };
    const clone=gltf.scene.clone(true); let index=0;
    clone.traverse((child) => { if(child instanceof THREE.Mesh){ const name=data.order[index++] ?? "building"; child.material=materials[name]; child.castShadow=name!=="glass"&&name!=="clear"; child.receiveShadow=true; } });
    ...
    clone.scale.setScalar(scale); clone.position.copy(center).multiplyScalar(-scale); clone.position.y+=size.y*scale*.03;
    return clone;
  },[data,gltf.scene,maps]);
  return <primitive object={scene}/>;
}

function UnitScene({type,controls}:{type:ModelType;controls:React.RefObject<OrbitControlsImpl|null>}){
  return <><ambientLight intensity={1.35}/><hemisphereLight intensity={.85} color="#fff" groundColor="#555"/><directionalLight castShadow intensity={1.5} position={[-6,10,9]} shadow-mapSize={[2048,2048]}/><Suspense fallback={null}><VillaModel type={type}/></Suspense><OrbitControls ref={controls} makeDefault target={[0,.45,0]} minDistance={5} maxDistance={22} enableDamping dampingFactor={.07}/></>;
}
